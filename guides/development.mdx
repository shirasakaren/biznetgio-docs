...
make build        # go build ./... - writes ./terraform-provider-biznetgio in the repo root, not a bin/ folder
...
cd pulumi-biznetgio
make provider           # builds only bin/pulumi-resource-biznetgio - just the Go provider binary
make build               # builds the provider AND regenerates + builds all five SDKs
...
make build_nodejs       # regenerate schema, rebuild the provider, then build the TypeScript SDK
```

<Warning>
  `make build` does not just build the Go provider - it also builds all five language SDKs (`dotnet_sdk go_sdk nodejs_sdk python_sdk java_sdk`), which needs `dotnet`, `yarn`/Node, `python3`, and `gradle` installed locally in addition to Go. If you only want the provider binary, use `make provider` instead. `make build_nodejs` also has hidden prerequisites: it regenerates the Pulumi schema first, which needs the `pulumi` CLI and `jq`, and rebuilds the provider binary before touching the nodejs SDK.
</Warning>
