...
Dua repo contoh nge-ship workflow GitHub Actions yang cuma jalan pas ada manusia yang sengaja nge-trigger. Halaman ini ngejelasin kenapa, tiap step ngapain, dan gimana cara nambahin contoh atau bahasa baru ke matrix. Situs docs gak punya file workflow sama sekali; dia deploy lewat aplikasi Mintlify.
...
Repo docs gak punya workflow Actions. Perubahan yang di-merge ke `main` di-build dan di-deploy oleh GitHub app Mintlify, yang dikonfigurasi di [deployments](https://github.com/shirasakaren/biznetgio-docs/deployments) repo. Verifikasi lokal sebelum merge itu `mint dev` plus cek link yang teliti, kayak yang dijelasin di [Local setup](/id/contribute/setup). Kalau deploy pernah keliatan basi, halaman riwayat deployment nunjukin commit hash dari situs yang lagi live.
