---
title: "NEO GPU (Pulumi)"
description: "Resource dan function Pulumi buat instance NEO GPU"
---

NEO GPU nyewain instance NVIDIA H200 (`/neo-gpus` API group), ditagih per cycle subscription atau on-demand (per jam). Harus set tepat satu billing mode - kalau nggak, validation gagal.

<Info>
  Setiap create bikin **order berbayar beneran**. Instance on-demand numpuk biaya per jam sampe di-delete.
</Info>

## `biznetgio:index:GpuInstance`

<Tabs>
  <Tab title="TypeScript">
    ```typescript
    import * as biznetgio from "@shirasakaren/biznetgio";

    const products = biznetgio.gpuProductsOutput();

    const keypair = new biznetgio.GpuKeypair("gpu-key", {
      name: "gpu-key",
    });

    const gpu = new biznetgio.GpuInstance("gpu-1", {
      productId: products.products[0].productId,
      selectOs: "ubuntu-22",
      keypairId: keypair.id.apply((id) => Number(id)),
      serviceName: "neo-gpu-1",
      sshAndConsoleUser: "root",
      consolePassword: "s3cretP4ssw0rd",
      payWithCreditCard: true,
      subscription: { cycle: "m" },
      // one-shot actions (change value to fire):
      // rebuildTrigger: "rebuild-1",
      // reserveAdditionalHoursTrigger: "reserve-1",
    });
    ```
  </Tab>
  <Tab title="Python">
    ```python
    import pulumi
    import pulumi_biznetgio

    products = pulumi_biznetgio.gpu_products()

    keypair = pulumi_biznetgio.GpuKeypair(
        "gpu-key",
        name="gpu-key",
    )

    gpu = pulumi_biznetgio.GpuInstance(
        "gpu-1",
        product_id=products.products[0].product_id,
        select_os="ubuntu-22",
        keypair_id=pulumi.Output.from_input(keypair.id).apply(int),
        service_name="neo-gpu-1",
        ssh_and_console_user="root",
        console_password="s3cretP4ssw0rd",
        pay_with_credit_card=True,
        subscription=pulumi_biznetgio.GpuInstanceSubscriptionArgs(cycle="m"),
        # rebuild_trigger="rebuild-1",
        # reserve_additional_hours_trigger="reserve-1",
    )
    ```
  </Tab>
  <Tab title="Go">
    ```go
    package main

    import (
        biznetgio "github.com/shirasakaren/pulumi-biznetgio/sdk/go/pulumi-biznetgio"
        "github.com/pulumi/pulumi/sdk/v3/go/pulumi"
    )

    func main() {
        pulumi.Run(func(ctx *pulumi.Context) error {
            products, err := biznetgio.GpuProducts(ctx, &biznetgio.GpuProductsArgs{})
            if err != nil {
                return err
            }

            keypair, err := biznetgio.NewGpuKeypair(ctx, "gpu-key", &biznetgio.GpuKeypairArgs{
                Name: pulumi.String("gpu-key"),
            })
            if err != nil {
                return err
            }

            gpu, err := biznetgio.NewGpuInstance(ctx, "gpu-1", &biznetgio.GpuInstanceArgs{
                ProductId:         pulumi.Int(products.Products[0].ProductId),
                SelectOs:          pulumi.String("ubuntu-22"),
                KeypairId:         keypair.ID().ToIntOutput(),
                ServiceName:       pulumi.String("neo-gpu-1"),
                SshAndConsoleUser: pulumi.String("root"),
                ConsolePassword:   pulumi.String("s3cretP4ssw0rd"),
                PayWithCreditCard: pulumi.Bool(true),
                Subscription: &biznetgio.GpuInstanceSubscriptionArgs{
                    Cycle: pulumi.String("m"),
                },
                // RebuildTrigger:                  pulumi.String("rebuild-1"),
                // ReserveAdditionalHoursTrigger:   pulumi.String("reserve-1"),
            })
            if err != nil {
                return err
            }

            ctx.Export("gpuStatus", gpu.Status)
            return nil
        })
    }
    ```
  </Tab>
  <Tab title=".NET">
    ```csharp
    using System;
    using System.Collections.Generic;
    using Pulumi;
    using Shirasakaren.Biznetgio;

    return await Deployment.RunAsync(() =>
    {
        var products = GpuProducts.Invoke();

        var keypair = new GpuKeypair("gpu-key", new GpuKeypairArgs
        {
            Name = "gpu-key",
        });

        var gpu = new GpuInstance("gpu-1", new GpuInstanceArgs
        {
            ProductId = products.Apply(p => p.Products[0].ProductId),
            SelectOs = "ubuntu-22",
            KeypairId = keypair.Id.Apply(v => int.Parse(v)),
            ServiceName = "neo-gpu-1",
            SshAndConsoleUser = "root",
            ConsolePassword = "s3cretP4ssw0rd",
            PayWithCreditCard = true,
            Subscription = new GpuInstanceSubscriptionArgs { Cycle = "m" },
            // RebuildTrigger = "rebuild-1",
            // ReserveAdditionalHoursTrigger = "reserve-1",
        });

        return new Dictionary<string, object?>
        {
            ["gpuStatus"] = gpu.Status,
        };
    });
    ```
  </Tab>
  <Tab title="Java">
    ```java
    package demo;

    import com.pulumi.Context;
    import com.pulumi.Pulumi;
    import ren.shirasaka.biznetgio.*;
    import ren.shirasaka.biznetgio.inputs.GpuSubscriptionArgs;

    public class App {
        public static void main(String[] args) {
            Pulumi.run(App::stack);
        }

        private static void stack(Context ctx) {
            var products = BiznetgioFunctions.gpuProducts();

            var keypair = new GpuKeypair("gpu-key",
                GpuKeypairArgs.builder()
                    .name("gpu-key")
                    .build());

            var gpu = new GpuInstance("gpu-1",
                GpuInstanceArgs.builder()
                    .productId(products.applyValue(p -> p.products().get(0).productId()))
                    .selectOs("ubuntu-22")
                    .keypairId(keypair.id().applyValue(Integer::parseInt))
                    .serviceName("neo-gpu-1")
                    .sshAndConsoleUser("root")
                    .consolePassword("s3cretP4ssw0rd")
                    .payWithCreditCard(true)
                    .subscription(GpuSubscriptionArgs.builder().cycle("m").build())
                    // .rebuildTrigger("rebuild-1")
                    // .reserveAdditionalHoursTrigger("reserve-1")
                    .build());

            ctx.export("gpuStatus", gpu.status());
        }
    }
    ```
  </Tab>
  <Tab title="YAML">
    ```yaml
    resources:
      gpu:
        type: biznetgio:index:GpuInstance
        properties:
          productId: ${products[0].productId}
          selectOs: ubuntu-22
          keypairId: ${fn::toNumber(keypair.id)}
          serviceName: neo-gpu-1
          sshAndConsoleUser: root
          consolePassword: s3cretP4ssw0rd
          payWithCreditCard: true
          subscription:
            cycle: m
    ```
  </Tab>
</Tabs>

Billing per jam sebagai ganti `subscription`:

<Tabs>
  <Tab title="TypeScript">
    ```typescript
    onDemand: { additionalHours: 0 },
    ```
  </Tab>
  <Tab title="Python">
    ```python
    on_demand=pulumi_biznetgio.GpuInstanceOnDemandArgs(additional_hours=0),
    ```
  </Tab>
  <Tab title="Go">
    ```go
    OnDemand: &biznetgio.GpuInstanceOnDemandArgs{
        AdditionalHours: pulumi.Int(0),
    },
    ```
  </Tab>
  <Tab title=".NET">
    ```csharp
    OnDemand = new GpuInstanceOnDemandArgs { AdditionalHours = 0 },
    ```
  </Tab>
  <Tab title="Java">
    ```java
    .onDemand(GpuOnDemandArgs.builder().additionalHours(0).build())
    ```
  </Tab>
  <Tab title="YAML">
    ```yaml
    onDemand:
      additionalHours: 0
    ```
  </Tab>
</Tabs>

| Input | Type | Required | Notes |
| --- | --- | --- | --- |
| `productId` | int64 | yes | dari `gpuProducts`; create-only |
| `selectOs` | string | yes | dari katalog select-OS produk; create-only |
| `keypairId` | int64 | yes | dari `GpuKeypair`; create-only |
| `sshAndConsoleUser` | string | yes | create-only |
| `consolePassword` | string, secret | yes | create-only |
| `serviceName` | string | no | create-only |
| `promocode` | string | no | create-only |
| `payWithCreditCard` | bool | no | default `true`; create-only |
| `subscription` | object | one-of | `{ cycle: "m" \| "a" }`; create-only |
| `onDemand` | object | one-of | `{ additionalHours?: number }` default 0; create-only |
| `rebuildTrigger` | string | no | one-shot reinstall OS (destructive) |
| `reserveAdditionalHoursTrigger` | string | no | one-shot: reserve additional hour(s) |

`keypairId` datengnya dari `id`-nya `GpuKeypair` yang di-convert ke number, kayak contoh di atas - `GpuKeypair` gak nge-expose output `keypairId` yang terpisah, beda dari resource keypair lain.

Outputs: `status`, `orderId`, `raw` (secret, di-redact). Import pake numeric account id.

<Note>
  Semua input di atas kecuali dua trigger itu create-only di Pulumi - kalau berubah, instance-nya di-destroy dan direcreate. Resource setara di Terraform provider gak nangani ini dengan aman: ganti `product_id`, `select_os`, atau `keypair_id` di situ nyebabin plan diff permanen yang gak bisa keselesein, bukan replace - liat [referensi Terraform](/id/terraform/resources/gpu#biznetgio_gpu_instance) kalau lu juga pake Terraform buat NEO GPU.
</Note>

## `biznetgio:index:GpuKeypair`

<Tabs>
  <Tab title="TypeScript">
    ```typescript
    const keypair = new biznetgio.GpuKeypair("gpu-key", {
      name: "gpu-key",
    });
    export const privateKey = keypair.privateKey;
    ```
  </Tab>
  <Tab title="Python">
    ```python
    keypair = pulumi_biznetgio.GpuKeypair(
        "gpu-key",
        name="gpu-key",
    )
    pulumi.export("privateKey", keypair.private_key)
    ```
  </Tab>
  <Tab title="Go">
    ```go
    keypair, err := biznetgio.NewGpuKeypair(ctx, "gpu-key", &biznetgio.GpuKeypairArgs{
        Name: pulumi.String("gpu-key"),
    })
    ctx.Export("privateKey", keypair.PrivateKey)
    ```
  </Tab>
  <Tab title=".NET">
    ```csharp
    var keypair = new GpuKeypair("gpu-key", new GpuKeypairArgs
    {
        Name = "gpu-key",
    });
    // keypair.PrivateKey
    ```
  </Tab>
  <Tab title="Java">
    ```java
    var keypair = new GpuKeypair("gpu-key",
        GpuKeypairArgs.builder()
            .name("gpu-key")
            .build());
    ctx.export("privateKey", keypair.privateKey());
    ```
  </Tab>
  <Tab title="YAML">
    ```yaml
    keypair:
      type: biznetgio:index:GpuKeypair
      properties:
        name: gpu-key
    ```
  </Tab>
</Tabs>

Input `name`; outputs `publicKey`, `privateKey` (secret - return cuma pas create), `raw` (secret). Gak ada update.

## Functions

<Note>
  Buat harga beneran dan cara milih paket tertentu, bukan asal percaya `[0]`, liat halaman [Katalog Produk](/id/products/gpu).
</Note>

### `biznetgio:index:gpuProducts`

Gak ada argument. Return `products`: `productId`, `name`, `description`, `categoryName`, `flavors` (`flavorId`, `name`, `raw`), `raw` (secret).

### `biznetgio:index:gpuConsole`

Argument `accountId` (string). Return `url` (secret) dan `raw` (secret). **Side-effecting:** tiap call bikin session console one-time baru - jangan dipake di dalam plan diffing.

### `biznetgio:index:gpuGraph`

Arguments `accountId` (string), `timeframe` (optional, default `hour`; `hour` / `day` / `week` / `month` / `year`). Return `graph` dan `raw` (secret) dari `GET /neo-gpus/accounts/{id}/graph-monitor`.
