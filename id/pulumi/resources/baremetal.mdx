...
| --- | --- | --- | --- |
| `productId` | int64 | yes | dari `baremetalProducts`; create-only - kalau berubah, server di-destroy dan direcreate |
| `cycle` | string | yes | `m` bulanan / `a` tahunan; create-only |
...
<Note>
  Pulumi nangani perubahan `productId` atau `cycle` dengan aman - destroy dan recreate server-nya. Resource setara di Terraform provider gak gitu - liat [referensi Terraform](/id/terraform/resources/baremetal#biznetgio_baremetal) kalau lu juga pake Terraform buat NEO Metal.
</Note>
...
| `cycle` | string | yes | `m` / `a`; liat warning di bawah |
...
<Warning>
  `cycle` itu satu-satunya input yang gak pernah dicek atau dikirim ke API sama sekali sama `Update` - beda dari `productId` dan `size`, `cycle` yang berubah ditulis ke state kayak berhasil, tanpa API call dan tanpa error, dan baik request change-package maupun upgrade emang gak punya field cycle buat itu. Kalau lu butuh billing cycle yang beda, destroy dan recreate resource-nya.
</Warning>
