...
Reference workspace yang aktif di dalam konfigurasi lu pake `terraform.workspace`. Ini cuma nunjukin bagian yang berubah aja; `data` lookup buat product dan OS image sama resource `biznetgio_neolite_keypair` itu persis yang udah dideklarasiin lengkap di [quickstart](/id/quickstart):
