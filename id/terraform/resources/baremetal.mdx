...
| `label` | string | required | display name - satu-satunya field yang beneran dikirim `apply` sebagai update |
...
<Warning>
  Ganti `product_id` atau `cycle` setelah create itu **gak ngapa-ngapain** - gak error, gak resize, gak re-billing. `Update` di Terraform cuma manggil API buat `label`, `power_state`, `reset_trigger`, dan `rebuild_os`; `product_id` atau `cycle` yang berubah langsung ditulis ke state tanpa nyentuh server, jadi `terraform apply` lapor sukses dan `plan` *berikutnya* gak nunjukin diff, padahal server bare-metal beneran masih di package dan cycle yang lama. Kalau lu butuh package atau billing cycle yang beda, lu harus destroy dan recreate resource-nya (atau ganti lewat [portal](https://portal.biznetgio.com)) - gak ada resize in-place buat NEO Metal.
</Warning>
...
<Note>
  `cycle` di sini bener create-only: kalau berubah, volume-nya di-destroy dan direcreate. Resource setara di [Pulumi provider](/id/pulumi/resources/baremetal#biznetgio:index:BaremetalElasticStorage) gak pernah ngecek atau ngirim `cycle` ke API sama sekali, in-place atau gimanapun - value yang berubah di situ ditulis diam-diam ke state kayak berhasil.
</Note>
