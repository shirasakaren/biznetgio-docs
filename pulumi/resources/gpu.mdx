...
| --- | --- | --- | --- |
| `productId` | int64 | yes | from `gpuProducts`; create-only |
| `selectOs` | string | yes | from the product's select-OS catalog; create-only |
| `keypairId` | int64 | yes | from `GpuKeypair`; create-only |
...
<Note>
  Every input above except the two triggers is create-only in Pulumi - a change destroys and recreates the instance. The Terraform provider's equivalent resource does not handle this safely: changing `product_id`, `select_os`, or `keypair_id` there causes a permanent, unresolvable plan diff instead of a replace - see the [Terraform reference](/terraform/resources/gpu#biznetgio_gpu_instance) if you also use Terraform for NEO GPU.
</Note>
