...
Reference the current workspace inside your configuration with `terraform.workspace`. The `biznetgio_neolite_keypair` resource is exactly what the [quickstart](/quickstart) already declared in full - what's different here is that `product_id` is no longer `products[0]`. It's picked deliberately per workspace, using the real package names from the [NEO Lite catalog](/products/neolite):
