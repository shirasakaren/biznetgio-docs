...
| `label` | string | required | display name - the only field `apply` actually sends as an update |
...
<Warning>
  Changing `product_id` or `cycle` after creation does **nothing** - no error, no resize, no re-billing. Terraform's `Update` only ever calls the API for `label`, `power_state`, `reset_trigger`, and `rebuild_os`; a changed `product_id` or `cycle` is written straight into state without touching the server, so `terraform apply` reports success and the *next* `plan` shows no diff, even though the real bare-metal server is still on the old package and cycle. If you need a different package or billing cycle, you must destroy and recreate the resource (or change it in the [portal](https://portal.biznetgio.com)) - there is no in-place resize for NEO Metal.
</Warning>
...
<Note>
  `cycle` is correctly create-only here: a change destroys and recreates the volume. The [Pulumi provider](/pulumi/resources/baremetal#biznetgio:index:BaremetalElasticStorage)'s equivalent resource never checks or sends `cycle` to the API at all, in place or otherwise - a changed value there is silently written into state as if it succeeded.
</Note>
